 ...
 */
 export function stepPathfinding(grid: Grid, pause: Function) {
  ...
  if (grid.pathTilePos === null) {
    ...
    if (grid.stepIndex >= 0) {
      ...
      grid.closedTiles.forEach((tile, i) => {
        //if (Math.abs(grid.pathTilePos.x - tile.x) > 1 ||  Math.abs(grid.pathTilePos.y - tile.y) > 1 || (grid.pathTilePos.x === tile.x && grid.pathTilePos.y === tile.y)) return
        if (Math.abs(grid.pathTilePos.x - tile.x) > 1 ||  Math.abs(grid.pathTilePos.y - tile.y) > 1) return
 ...
 */
 function calculateAdjacentNodes(grid: Grid, x: number, y: number, targetX: number, targetY: number, addG: number) {
  ...
  if (grid.openTiles.length > 0) {
    ...
    grid.closedTiles.push(grid.openTiles.shift() as PathTile)                  // remove lowest f cost openTile and add it to closedTiles
  }
  ...
  for (let j = 0; j < 8; j++) {
    ...
    if (neighborTiles[j] === 0) {   // for each open neighbor tile
      ...
      if (grid.openTiles.length > 0) {
        grid.openTiles.push(new PathTile(xPos, yPos, g, h, reference))
        grid.openTiles.sort((a: PathTile, b: PathTile) => {
          const dif = a.g + a.h - b.g - b.h
          return !dif ? a.g + a.h : dif
        })
      } else grid.openTiles.push(new PathTile(xPos, yPos, g, h, reference))

    } else if (neighborTiles[j] === -1) {  // refactor self to other open tile
      for (let k = 0; k < grid.openTiles.length; k++) {
        if (grid.openTiles[k].x === xPos 
            && grid.openTiles[k].y === yPos 
            && g < grid.openTiles[k].g) {
          grid.openTiles[k].g = g

          grid.openTiles.sort((a: PathTile, b: PathTile) => a.g + a.h - b.g - b.h)
          break
        }
      }
    }
  ...
  if (Math.abs(targetX - x) < 2 && Math.abs(targetY - y) < 2) 
    grid.pathTilePos = new Position(targetX, targetY)
}
